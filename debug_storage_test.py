...
def test_file_storage():
    ...
    try:
        storage = StorageEngine(temp_path)
        print(f"DB Path: {storage.db_path}")
        ...
        note = ParsedNote(
            note_id=str(uuid.uuid4()),
            tags=["test", "file"],
            note="Test note for file storage",
            comment="Testing file storage behavior",
            timestamp=datetime.now(timezone.utc).isoformat(),
            version=1,
            semantic_density=0.8,
            tag_quality_score=0.9,
            ...
            issues=[],
            origin="debug",
            raw_text="test, file: Test note for file storage : Testing file storage behavior",
            tag_hierarchy={},
            co_occurrence_patterns={}
        )
        
        # Test storage
        result = storage.store_note(note)
